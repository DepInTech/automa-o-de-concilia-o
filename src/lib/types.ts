...
// Retorna verdadeiro se os valores forem idênticos ou dentro da tolerância de R$ 2,00 (Uber)
...
export function reconcileData(
  ...
  // 1. Processa todos os registros do sistema buscando correspondentes
  for (const sys of systemRecords) {
    ...
    // Tenta encontrar um match perfeito ou dentro da tolerância de R$ 2,00 -> VERDE
    ...
    // Se não encontrou GREEN, escolhe o candidato de mesmo estabelecimento com a MENOR diferença de valor -> AMARELO
  ...
  // 2. Registros que ficaram apenas no Sistema -> RED
  for (const sys of systemRecords) {
    if (matchedSystem.has(sys.id)) continue
  ...
  // 3. Registros que ficaram apenas na Fatura -> RED
